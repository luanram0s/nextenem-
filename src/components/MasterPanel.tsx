...
const stats = [
  { label: 'Alunos Ativos', value: '1,284', trend: '+12%', icon: Users, color: 'text-blue-500', bg: 'bg-blue-500/10' },
  { label: 'Simulados Realizados', value: '8,432', trend: '+24%', icon: ClipboardCheck, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
  { label: 'Dúvidas Pendentes', value: '12', trend: '-5%', icon: MessageSquare, color: 'text-amber-500', bg: 'bg-amber-500/10' },
];
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 pb-20">
      <div className="mb-8">
        <button 
          onClick={() => navigate('/')}
          className="flex items-center gap-2 text-zinc-400 hover:text-zinc-950 font-black text-[10px] uppercase tracking-widest transition-all duration-300 mb-4 group"
        >
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex bg-zinc-100 p-1.5 rounded-2xl">
          <button className="px-6 py-2.5 bg-white shadow-sm rounded-xl text-xs font-black uppercase tracking-widest text-zinc-950 transition-all duration-300">Visão Geral</button>
          <button className="px-6 py-2.5 text-xs font-black uppercase tracking-widest text-zinc-400 hover:text-zinc-600 transition-all duration-300">Relatórios</button>
        </div>
      </header>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {stats.map((stat, i) => (
          <div key={i} className="bg-white p-8 rounded-2xl border border-zinc-100 shadow-sm hover:shadow-xl hover:shadow-zinc-200/50 transition-all duration-300 hover:scale-[1.01] group">
            <div className="flex justify-between items-start mb-6">
              <div className={`w-14 h-14 ${stat.bg} rounded-xl flex items-center justify-center ${stat.color} group-hover:scale-110 transition-transform duration-300`}>
                <stat.icon size={26} />
              </div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-100 p-10 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between mb-10">
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="pl-12 pr-6 py-4 bg-zinc-50 border-none rounded-2xl text-xs font-bold focus:ring-2 focus:ring-blue-600 min-w-[300px] transition-all duration-300"
            />
          </div>
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="text-[10px] font-black text-zinc-400 uppercase tracking-widest border-b border-zinc-50">
                <th className="pb-6 px-6">Estudante</th>
                <th className="pb-6 px-6">Plano</th>
                <th className="pb-6 px-6 text-center">Progresso</th>
                <th className="pb-6 px-6 text-center">Status</th>
                <th className="pb-6 px-6 text-right">Ações</th>
              </tr>
            </thead>
              ...
              {students.map((student) => (
                <tr key={student.id} className="group hover:bg-zinc-50/50 transition-colors">
                  ...
                  <td className="py-6 px-6">
                    <div className="flex items-center justify-center gap-3">
                      <div className="w-24 h-1.5 bg-zinc-100 rounded-full overflow-hidden">
                        <div className="h-full bg-blue-600 rounded-full transition-all duration-500" style={{ width: student.progress }} />
                      </div>
                      <span className="text-[10px] font-black text-zinc-950 min-w-[30px]">{student.progress}</span>
                    </div>
                  </td>
                  ...
                  <td className="py-6 px-6 text-right">
                    <button className="p-2 text-zinc-300 hover:text-zinc-950 transition-colors duration-300">
